...
-- 0) Renaming some columns to improve redability
ALTER TABLE solar_data
CHANGE COLUMN `H(i)_m` irradiation_tilt_month DOUBLE;

ALTER TABLE solar_data
CHANGE COLUMN `T2m` temp DOUBLE;
...
FROM
	peruvian_cities LEFT JOIN solar_data ON peruvian_cities.city_index = solar_data.city_index
GROUP BY
	city,
    angle,
    `year`
...
SET @city_name = 'Chiclayo'; -- This is a variable
